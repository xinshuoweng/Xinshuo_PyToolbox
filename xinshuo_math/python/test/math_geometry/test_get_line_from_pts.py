...
def test_get_2Dline_from_pts():
	...
	assert CHECK_EQ_NUMPY(line, np.array([1, 1, -1]).reshape((1, 3)))
	
	print('check vertical line')
	pts1 = [1, 1, 1]
	pts2 = [1, 12, 1]
	line = get_2Dline_from_pts(pts1, pts2)
	assert CHECK_EQ_NUMPY(line, np.array([-11, 0, 11]).reshape((1, 3)))

	print('check horizontal line')
	pts1 = [20, 0, 1]
	pts2 = [1, 0, 1]
	line = get_2Dline_from_pts(pts1, pts2)
	assert CHECK_EQ_NUMPY(line, np.array([0, -19, 0]).reshape((1, 3)))

	print('check a point at infinity')
	pts1 = [0, 3, 0]
	pts2 = [1, 0, 1]
	line = get_2Dline_from_pts(pts1, pts2)
	assert CHECK_EQ_NUMPY(line, np.array([3, 0, -3]).reshape((1, 3)))

	print('check line at infinity')
	pts1 = [0, 3, 0]
	pts2 = [1, 0, 0]
	line = get_2Dline_from_pts(pts1, pts2)
	assert CHECK_EQ_NUMPY(line, np.array([0, 0, -3]).reshape((1, 3)))
