...
def safe_angle(input_angle, debug=True):
	'''
...
def safe_pts(input_pts, debug=True):
	'''
	...
	if islist(input_pts):
		if islistoflist(input_pts):
			if debug:
				assert all(len(list_tmp) == 2 for list_tmp in input_pts), 'all sub-lists should have length of 2'
			np_pts = np.array(input_pts).transpose()
		else:
			if debug:
				assert len(input_pts) == 2, 'the input pts list does not have a good shape'
		...
		if input_pts.shape == (2, ):
			...
			if debug:
				assert is2dptsarray(input_pts), 'the input pts array does not have a good shape'
			np_pts = input_pts
	else:
