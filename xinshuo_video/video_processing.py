...
def extract_images_from_video_ffmpeg(video_file, save_dir, format='frame%06d.png', startnum=0, verbose=True, debug=True):
	...
	mkdir_if_missing(save_dir)
	if verbose:
		command = 'ffmpeg -i %s -start_number %d %s/%s' % (video_file, start_number, save_dir, format)
	else:
		command = 'ffmpeg -loglevel panic -i %s -start_number %d %s/%s' % (video_file, start_number, save_dir, format)
	os.system(command)
