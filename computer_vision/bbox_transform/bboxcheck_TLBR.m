...
% input format: TLBR 
% output format: TLBR 
function boxes = bboxcheck_LTBR(boxes)
	if ischar(boxes)
		try
	...
	test_TLBR = [boxes(:, 3) - boxes(:, 1), boxes(:, 4) - boxes(:, 2)];
	assert(sum(sum(test_TLBR < 0)) == 0, 'The input format for boxes should be LTBR, two points (left top and bottom right).');
