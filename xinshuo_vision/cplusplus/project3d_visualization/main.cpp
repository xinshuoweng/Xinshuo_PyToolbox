...
int main(int argc, char* argv[]){
    if( argc != 9 ){
        fprintf(stderr, "calibration pose_dir mesh num_pts camera_src frame pose3dout_dir resize_factor\n");
        return -1;
    }
...
//    ASSERT_WITH_MSG(camera_cluster.size() == 1, "multiview is not supported now!");
    ...
    std::vector<std::vector<pts_3d_conf>> pts_3d_allviews(camera_cluster.size(), std::vector<pts_3d_conf>(num_pts, pts_3d_conf()));    // view, num_pts, pts_3d_conf
...
//     for visualization
    ...
    sprintf(cmd, "mkdir -p %s/pose3d", argv[7]);
    ret = system(cmd);
    ASSERT_WITH_MSG(ret == 0, "creating folder for 3d locations failed!");
    sprintf(cmd, "%s/pose3d/%05d.pose", argv[7], frame);
    ...
    for (int camera_index = 0; camera_index < camera_cluster.size(); camera_index++) {
        ...
        while(fscanf(in, "%lf %lf %lf\n", &x, &y, &conf) > 0){
            ...
            pts_3d_allviews[camera_index][pts_index] = pts_3d_tmp;
            pts_index++;
...
//            }
            ...
            P.at<double>(0, 0) = pts_3d_allviews[camera_index][pts_index].x;
            P.at<double>(1, 0) = pts_3d_allviews[camera_index][pts_index].y;
            P.at<double>(2, 0) = pts_3d_allviews[camera_index][pts_index].z;
            P.at<double>(3, 0) = 1;
            P = camera_tmp.getProjectionMatrix() * P;       // projection 3d to 2d
            fprintf(out, "%g %g %g\n", P.at<double>(0, 0) / P.at<double>(2, 0) / resize_factor, P.at<double>(1, 0) / P.at<double>(2, 0) / resize_factor, pts_3d_allviews[camera_index][pts_index].conf);
        }
        fclose(out);
    }
    return 0;
