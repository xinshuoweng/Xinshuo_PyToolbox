...
from xinshuo_math.python.private import safe_npdata, safe_angle, safe_ptsarray, safe_ptsarray_occlusion
...
def generate_gaussian_heatmap(input_pts, image_size, std, warning=True, debug=True):
	...
	mask_invalid = np.ones((1, 1, num_pts+1), dtype='float32')
	mask_invalid[0, 0, :num_pts] = valid
	mask_invalid[0, 0, num_pts]  = 1										# never mask out the background channel

	heatmap = np.exp(heatmap)
	heatmap[heatmap < threshold] = 0						# ceiling and flooring
	heatmap[heatmap >         1] = 1
	heatmap = heatmap * mask_invalid[:, :, :num_pts]		# (height, width, num_pts)

	background_label = 1 - np.amax(heatmap, axis=2)
	background_label[background_label < 0] = 0									# (height, width, 1)
	heatmap = np.concatenate((heatmap, np.expand_dims(background_label, axis=2)), axis=2).astype('float32')

	return heatmap*mask_invalid, mask_invalid
