...
def generate_gaussian_heatmap(input_pts, image_size, std, downsample_factor=1.0, warning=True, debug=True):
	'''
	...
	# 0 -> occluded without annotation, 1 -> visible with annotation, -1 -> visible without annotation
	# only points which have third row of -1 will be masked out as they are not valid
	valid = np.logical_or(pts_array[2, :] == 0 or pts_array[2, :] == 1)	
	mask_heatmap = np.ones((1, 1, num_pts+1), dtype='float32')
	mask_heatmap[0, 0, :num_pts] = valid
	mask_heatmap[0, 0, num_pts]  = 1
	...
	transformed_label = transformed_label * mask_heatmap[:, :, :num_pts]
